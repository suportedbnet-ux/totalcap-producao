...
import { Plus, Search, Edit2, Trash2, X, ShieldAlert } from 'lucide-react';
...
export default function TabRecusa() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const response = await api.get('/tabrecusa/');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (modalMode === 'create') {
        await api.post('/tabrecusa/', formData);
      } else {
        await api.put(`/tabrecusa/${currentId}`, formData);
      }
    ...
    <div className="page-container">
      <div className="page-header">
        <div className="header-title-group">
          <ShieldAlert className="header-icon" style={{ color: '#f59e0b' }} />
          <div>
            <h1>Motivos de Recusa</h1>
            <p>Gerenciamento da tabela de motivos para recusa de garantia</p>
          </div>
        </div>
