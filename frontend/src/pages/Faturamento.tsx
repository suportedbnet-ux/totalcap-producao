...
import { 
  ...
  TrendingUp, CheckCircle, Clock, Plus, Trash2, X, Settings, Activity, Wrench, Calendar, Home, Book, Edit, Eye, Download
} from 'lucide-react';
...
export default function Faturamento() {
  ...
  const [activeTab, setActiveTab] = useState<'informe' | 'faturas'>('faturas');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');
  const [empresa, setEmpresa] = useState<any>(null);

  // Aba 1: Informe de Serviços removido. Mantida seleção de Faturas apenas.
  ...
  // Pneus and related services state for pneu workflow
  const [pneuResults, setPneuResults] = useState<any[]>([]);
  const [pneuServicos, setPneuServicos] = useState<any[]>([]);
  const [allServicos, setAllServicos] = useState<any[]>([]);
  const [newServico, setNewServico] = useState<{ id_servico: number; quant: number; valor: number }>({ id_servico: 0, quant: 1, valor: 0 });
  const [editingServico, setEditingServico] = useState<any>(null);
  const [servicoSearchQuery, setServicoSearchQuery] = useState<string>('');
  const [showServicoSuggestions, setShowServicoSuggestions] = useState<boolean>(false);
  const [isServicoModalOpen, setIsServicoModalOpen] = useState<boolean>(false);
  const [pneuSearchQuery, setPneuSearchQuery] = useState<string>('');
  
  // Serviços adicionais removidos (aba Informe)
  ...
  const [nfseToPrint, setNfseToPrint] = useState<any | null>(null);
  ...
  const handlePrintNFSe = (fatura: any) => {
    const dataToPrint = {
      ...fatura,
      ...faturaForm,
      pneus: pneuServicosPreview,
    ...
    setNfseToPrint(dataToPrint);
    document.body.classList.add('printing-single-nfse-active');
    
    setTimeout(() => {
      window.print();
      setTimeout(() => {
        setNfseToPrint(null);
        document.body.classList.remove('printing-single-nfse-active');
      }, 500);
    }, 100);
  };
  ...
  const fetchFaturas = async () => {
    try {
      ...
      const params: any = { limit: 500, tipofat: 'S' };
      if (faturaSearchQuery) params.q = faturaSearchQuery;
      if (faturaStartDate) params.start_date = faturaStartDate;
      if (faturaEndDate) params.end_date = faturaEndDate;
  ...
  const handleOpenFaturaModal = async (fatura?: any, mode: 'create' | 'edit' | 'view' = 'create') => {
    ...
    if (fatura) {
      // Se estiver editando, busca dados completos do banco para garantir pneus carregados
      let faturaCompleta = fatura;
      if (mode === 'edit' || mode === 'view') {
        try {
          const res = await api.get(`/faturas/${fatura.id}`);
          faturaCompleta = res.data;
        } catch (err) {
          console.error("Erro ao buscar fatura completa:", err);
        }
      }

      setEditingFatura(faturaCompleta);
      setFaturaForm({
        id_contato: faturaCompleta.id_contato,
        cliente_nome: faturaCompleta.contato_nome || '',
        id_planopag: faturaCompleta.id_planopag,
        id_vendedor: faturaCompleta.id_vendedor,
        id_banco: faturaCompleta.id_banco,
        id_tipodocto: faturaCompleta.id_tipodocto,
        id_fatura_erp: faturaCompleta.id_fatura_erp,
        obs: faturaCompleta.obs || '',
        datafat: (faturaCompleta.datafat || new Date().toISOString()).split('T')[0],
        vrservico: parseFloat(faturaCompleta.vrservico || 0),
        vrproduto: parseFloat(faturaCompleta.vrproduto || 0),
        vrcarcaca: parseFloat(faturaCompleta.vrcarcaca || 0),
        vrmontagem: parseFloat(faturaCompleta.vrmontagem || 0),
        vrbonus: parseFloat(faturaCompleta.vrbonus || 0),
        vrtotal: parseFloat(faturaCompleta.vrtotal || 0)
      });
      console.log("DEBUG handleOpenFaturaModal: pneus carregados =", faturaCompleta.pneus);
      setSelectedPneusForFatura(faturaCompleta.pneus?.map((p: any) => p.id) || []);
      console.log("DEBUG handleOpenFaturaModal: selectedPneusForFatura =", faturaCompleta.pneus?.map((p: any) => p.id) || []);
      setPneuServicosPreview(faturaCompleta.items || []);
      setFaturaParcelasPreview(faturaCompleta.parcelas || []);
      fetchFaturaLaudos(faturaCompleta.id);
      ...
      setSearchParams({ id: '', numos: '', cliente: '', id_pneu: '' }); // Limpa os filtros
  ...
  const handleSelectAllPneus = () => {
    ...
    const allIds = osResults.map(p => Number(p.pneu_id));
  ...
  const handleProceedToFaturaDetails = async () => {
    ...
    try {
      ...
      const selectedData = osResults.filter(p => selectedPneusForFatura.includes(Number(p.pneu_id)));
  ...
  // Calcular parcelas para preview com base no plano de pagamento
  const calculateParcelasPreview = (forcedVrServico?: number, forceRegen: boolean = false) => {
    // Verificação explícita para id_planopag (não usar ! que retorna true para 0)
    const planopagId = faturaForm.id_planopag;
    const hasPlanopag = planopagId !== null && planopagId !== undefined && planopagId !== 0;

    if (!isFaturaModalOpen) return;
    if (!hasPlanopag) {
      console.log('DEBUG: id_planopag não definido ou é 0');
      return;
    }
    
    // Bloqueia somente se a fatura já tiver sido enviada para o ERP
    if (faturaForm.id_fatura_erp && faturaForm.id_fatura_erp !== 'null' && faturaForm.id_fatura_erp !== '0') {
      console.log('DEBUG: fatura já exportada para o ERP, não regera parcelas');
      return;
    }

    // Não regera parcelas se estiver editando uma fatura existente (a não ser que forçado pelo botão)
    if (editingFatura && !forceRegen) {
      console.log('DEBUG: editando fatura existente, não regera parcelas automaticamente');
      return;
    }

    const plano = allPlanosPag.find(p => p.id === planopagId);
    if (!plano || !plano.numparc) {
      console.log('DEBUG: plano não encontrado ou sem numparc', plano);
      return;
    }

    const vrServicoVal = forcedVrServico !== undefined ? forcedVrServico : parseFloat(faturaForm.vrservico || 0);

    const totalLaudos = faturaLaudosPreview.reduce((acc, curr) => acc + parseFloat(curr.valor || 0), 0);
    const total =
      vrServicoVal +
      parseFloat(faturaForm.vrproduto || 0) +
      parseFloat(faturaForm.vrcarcaca || 0) +
      parseFloat(faturaForm.vrmontagem || 0) -
      parseFloat(faturaForm.vrbonus || 0) -
      totalLaudos;
    ...
    const intervalo = plano.intervalo || 30; // Usa o intervalo do plano ou 30 dias como padrão
    const valorParcelaBase = parseFloat((total / numparc).toFixed(2));
    const parcelas: any[] = [];
    const dataBase = new Date(faturaForm.datafat + 'T12:00:00'); // midday to avoid timezone issues
    let valorAcumulado = 0;

    for (let i = 1; i <= numparc; i++) {
      const venc = new Date(dataBase);
      // Usa o intervalo do plano: primeira parcela vence em 'intervalo' dias, segunda em 2*intervalo, etc.
      const daysToAdd = i * intervalo;
      venc.setDate(venc.getDate() + daysToAdd);

      // A última parcela recebe o valor restante para evitar erros de arredondamento
      let valorParcela = valorParcelaBase;
      if (i === numparc) {
        valorParcela = parseFloat((total - valorAcumulado).toFixed(2));
      } else {
  ...
  // Chamado automaticamente quando valores mudam (para preview em tempo real)
  useEffect(() => {
    // Só recalcula se tiver um plano selecionado e não estiver editando
    if (faturaForm.id_planopag && !editingFatura && isFaturaModalOpen) {
      calculateParcelasPreview();
    }
  }, [
    faturaForm.id_planopag,
    faturaForm.id_tipodocto,
  ...
  // Função separada para forçar regeneração (chamada pelo botão)
  const handleGerarParcela = () => {
    if (!faturaForm.id_planopag) {
      alert('Selecione um Plano de Pagamento');
      return;
    }
    if (faturaForm.id_fatura_erp && faturaForm.id_fatura_erp !== 'null' && faturaForm.id_fatura_erp !== '0') {
      alert('Não é possível regenerar parcelas de uma fatura que já foi exportada para o ERP. Faça os ajustes no GestãoClick.');
      return;
    }
    if (allPlanosPag.length === 0) {
      alert('Planos de pagamento não carregados. Aguarde e tente novamente.');
      return;
    }
    
    // Refaz o total da fatura a partir de todos os pneus selecionados
    const newVrServico = pneuServicosPreview.reduce((acc, curr) => acc + parseFloat(curr.vrtotal || 0), 0);
    const newVrTotal = newVrServico + parseFloat(faturaForm.vrproduto || 0) + parseFloat(faturaForm.vrcarcaca || 0) + parseFloat(faturaForm.vrmontagem || 0) - parseFloat(faturaForm.vrbonus || 0);

    setFaturaForm((prev: any) => ({
      ...prev,
      vrservico: newVrServico,
      vrtotal: newVrTotal
    }));

    // Chama o recálculo forçando a regeneração e usando o novo vrservico calculado
    calculateParcelasPreview(newVrServico, true);
  };
  ...
  const handleSaveFatura = async () => {
    ...
    // Só exige pneus quando estiver criando uma fatura nova
    if (!editingFatura && selectedPneusForFatura.length === 0) {
      alert("Selecione ao menos um pneu.");
      return;
    }
    console.log("DEBUG handleSaveFatura: editingFatura =", !!editingFatura, "pneus =", selectedPneusForFatura);
    ...
    try {
      ...
      let errorMessage = "Erro desconhecido ao salvar fatura.";
      
      if (typeof detail === 'string') {
        errorMessage = detail;
      } else if (Array.isArray(detail)) {
        // Formata os erros do Pydantic para exibir qual campo deu problema
        errorMessage = detail.map((d: any) => {
          const field = d.loc ? d.loc[d.loc.length - 1] : 'Desconhecido';
          return `• Campo [${field}]: ${d.msg || d.message}`;
        }).join('\n');
      ...
      alert(`⚠️ Erro de Validação ou Conexão ao Salvar:\n\n${errorMessage}`);
    }
  };
  ...
  const handleOSSearch = async (e: React.FormEvent) => {
    ...
    setError('');
    setOsResults([]);
    setSelectedPneusForFatura([]);  // Limpa seleções de buscas anteriores para evitar serviços de pneus não selecionados
  ...
  const handleImportarERP = async () => {
    alert("Função de importação do ERP em desenvolvimento. Por favor, forneça os parâmetros desejados para a integração.");
  };

  const handleExportarERP = async (faturaIdOverride?: number) => {
    const idsToExport = faturaIdOverride ? [faturaIdOverride] : selectedFaturaIds;
    if (idsToExport.length === 0) {
      alert("Selecione ao menos uma fatura para exportar.");
      return;
    }
    ...
    try {
      ...
      for (const faturaId of idsToExport) {
        // Busca a fatura fresquinha do banco para garantir que a tabela fatura_servico (items) venha preenchida
        const faturaRes = await api.get(`/faturas/${faturaId}`);
        const fatura = faturaRes.data;
        if (!fatura) continue;

        const payloadParam = {
          "cliente_id": String(fatura.id_contato_erp || fatura.id_contato || ""),
          "vendedor_id": String(fatura.id_vendedor_erp || fatura.id_vendedor || ""),
          "nome_vendedor": fatura.vendedor_nome || "",
          "data_entrada": fatura.datafat ? fatura.datafat.split('T')[0] : "",
          "data_saida": fatura.datafat ? fatura.datafat.split('T')[0] : "",
          "situacao_id": "9184469",
          ...
          "observacoes": fatura.obs || "",
          "observacoes_interna": `[Totalcap_id=${fatura.id}]`,
          "valor_frete": "0.00",
          ...
          "pagamentos": (fatura.parcelas || []).map((parcela: any) => ({
            "pagamento": {
              "data_vencimento": parcela.vencto ? parcela.vencto.split('T')[0] : "",
              "valor": String(parcela.valor || "0.00"),
              "forma_pagamento_id": "6607625",
              "nome_forma_pagamento": "Duplicata Mercantil",
              "plano_contas_id": "35647961",
              "nome_plano_conta": "Prestações de serviços",
              "observacao": ""
            }
          })),
          "servicos": (fatura.items && fatura.items.length > 0) ? fatura.items.map((item: any) => {
            const srv: any = {
              "servico_id": String(item.id_servico_erp || ""),
              "nome_servico": "Recapagem",
              "detalhes": item.descricao || "Serviço",
              "quantidade": String(item.quant || "1"),
              "valor_venda": String(item.valor || "0.00"),
              "desconto": String(item.vrdescto || "0.00"),
              "observacao": ""
            };
            return { "servico": srv };
          }) : (fatura.pneus || []).map((pneu: any) => ({
            "servico": {
              "servico_id": "",
              "nome_servico": "Recapagem",
              "detalhes": "Serviço de Pneu " + (pneu.numfogo || ""),
              "quantidade": "1",
              "valor_venda": String(pneu.vrservico || pneu.valor || "0.00"),
              "desconto": "0.00",
              "observacao": `Série: ${pneu.numserie || ""}`
            }
          }))
        };

        // Remove array pagamentos se estiver vazio para evitar Undefined offset 0
        if (payloadParam.pagamentos && payloadParam.pagamentos.length === 0) {
          delete payloadParam.pagamentos;
          payloadParam.condicao_pagamento = "a_vista";
        }

        // Remove array servicos se estiver vazio
        if (payloadParam.servicos && payloadParam.servicos.length === 0) {
          delete payloadParam.servicos;
        }

        const payloadRequest = {
          recurso: "ordens_servicos",
          method: "POST",
          param: payloadParam
        };
        
        payloads.push(payloadRequest);

        const response: any = await api.post('/gestaoclick/proxy', payloadRequest);
        const responseData = response.data?.response_data || response.data;
        responses.push(responseData);
        headersList.push(response.data?.request_info?.headers || {});
        urlList.push(response.data?.request_info?.url || "URL Desconhecida");

        let erpId = null;
        if (responseData?.data?.[0]?.id) erpId = responseData.data[0].id;
        else if (responseData?.data?.id) erpId = responseData.data.id;
        else if (Array.isArray(responseData) && responseData[0]?.id) erpId = responseData[0].id;
        else if (responseData?.id) erpId = responseData.id;

        if (erpId) {
          if (editingFatura?.id === faturaId) {
            // Se estiver na tela de edição, simula o "Salvar Alterações" completo com o novo ID
            const payloadToSave = {
              ...faturaForm,
              id_fatura_erp: String(erpId),
              tipofat: 'S',
              pneu_ids: selectedPneusForFatura,
              parcelas: faturaParcelasPreview,
              vrtotal: (faturaForm.vrservico + faturaForm.vrproduto + faturaForm.vrcarcaca + faturaForm.vrmontagem - faturaForm.vrbonus - faturaLaudosPreview.reduce((acc: any, curr: any) => acc + parseFloat(curr.valor || 0), 0))
            };
            await api.put(`/faturas/${faturaId}`, payloadToSave);
            setEditingFatura({ ...editingFatura, id_fatura_erp: String(erpId) });
            setFaturaForm((prev: any) => ({ ...prev, id_fatura_erp: String(erpId) }));
          } else {
            // Caso contrário (exportando pela lista), apenas faz um patch do ID
            await api.put(`/faturas/${faturaId}`, { id_fatura_erp: String(erpId) });
          }

          if (selectedFatura?.id === faturaId) {
            setSelectedFatura({ ...selectedFatura, id_fatura_erp: String(erpId) });
          }
        }
      }
      ...
      const jsonPayload = JSON.stringify(payloads.length === 1 ? payloads[0] : payloads, null, 2);
      const jsonResponse = JSON.stringify(responses.length === 1 ? responses[0] : responses, null, 2);
      const jsonHeaders = JSON.stringify(headersList.length === 1 ? headersList[0] : headersList, null, 2);
      const reqUrl = urlList.length === 1 ? urlList[0] : urlList.join(", ");

      console.log("========== JSON DA REQUISIÇÃO ==========");
      console.log(jsonPayload);
      console.log("========== RETORNO DA API ==========");
      console.log(jsonResponse);

      const debugText = `STATUS: 200\nURL: ${reqUrl}\n\n-----REQUISICAO ENVIADO -----\n${jsonPayload}\n\n-------RETORNO DE REQUISICAO-----\n${jsonResponse}`;
      ...
      alert("Fatura(s) exportada(s) com sucesso para o ERP!");

    } catch (err: any) {
      const errorResponse = err.response?.data || err.message;
      const statusCode = err.response?.status || "DESCONHECIDO";
      const jsonPayload = payloads.length > 0 ? JSON.stringify(payloads.length === 1 ? payloads[0] : payloads, null, 2) : "Requisição não montada";
      const jsonError = JSON.stringify(errorResponse, null, 2);
      const errHeaders = err.response?.data?.request_info?.headers ? JSON.stringify(err.response.data.request_info.headers, null, 2) : "{}";
      const errUrl = err.response?.data?.request_info?.url || "URL Desconhecida";
      ...
      try {
        await navigator.clipboard.writeText(debugText);
        alert(`Erro ao exportar faturas.\n\nO JSON de envio e o detalhe do erro foram copiados para a sua área de transferência para facilitar a análise.`);
      } catch (e) {
        alert(`Erro ao exportar faturas: ${err.message}. Verifique os detalhes na tela.`);
      }
    }
  };
  ...
  return (
    <div className="faturamento-container">
      ...
      {/* Aba de Faturas (painel de navegação removido conforme solicitado) */}
      {error && (
      ...
      {apiResponseInfo && (
        <div className="premium-master-panel animate-fade-in" style={{ margin: '1.5rem', background: '#1e293b', color: '#f8fafc', padding: '1.5rem', borderRadius: '12px', border: '1px solid #334155' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid #334155', paddingBottom: '0.8rem' }}>
            <span style={{ fontWeight: 'bold', color: '#38bdf8', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
               Dados da Integração ERP (Ordens de Serviço GestãoClick)
            </span>
            <button type="button" onClick={() => setApiResponseInfo(null)} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: '#f8fafc', cursor: 'pointer', padding: '0.3rem', borderRadius: '4px' }}>
              <X size={16} />
            </button>
          </div>
          <pre style={{ margin: 0, fontFamily: 'monospace', fontSize: '0.85rem', whiteSpace: 'pre-wrap', maxHeight: '400px', overflowY: 'auto' }}>
            {apiResponseInfo}
          </pre>
        </div>
      )}
      ...
      {isServicoModalOpen && (
        <div className="os-modal-overlay" onClick={() => setIsServicoModalOpen(false)}>
          ...
          {activeTab === 'faturas' && (
            <div className="tab-content animate-fade-in" style={{ marginTop: '-1rem' }}>
            <div className="search-section glass-panel fat-search-section" style={{ padding: '0.5rem 1rem', marginBottom: '1rem' }}>
            <div className="form-group fat-search-group" style={{ marginBottom: 0 }}>
              <label style={{ marginBottom: '0.25rem', fontSize: '0.85rem' }}><Search size={14} /> Buscar Fatura (Número ou Cliente)</label>
              <div className="fat-inputs-row" style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                <input
                  type="text"
                  className="form-input fat-input"
                  ...
                  style={{ fontSize: '0.9rem', boxSizing: 'border-box', height: '32px' }}
                />
                <input
                  type="date"
                  className="form-input fat-input"
                  value={faturaStartDate}
                  onChange={(e) => setFaturaStartDate(e.target.value)}
                  style={{ boxSizing: 'border-box', height: '32px', fontSize: '0.9rem' }}
                />
                <input
                  type="date"
                  className="form-input fat-input"
                  value={faturaEndDate}
                  onChange={(e) => setFaturaEndDate(e.target.value)}
                  style={{ boxSizing: 'border-box', height: '32px', fontSize: '0.9rem' }}
                />
                <button
                  className="btn-search-producao fat-filter-btn"
                  onClick={fetchFaturas}
                  style={{ whiteSpace: 'nowrap', display: 'flex', alignItems: 'center', boxSizing: 'border-box', height: '32px', padding: '0 1rem' }}
                >
                  {faturaLoading ? <span>Buscando...</span> : <><Search size={16} style={{ marginRight: '4px' }} /> <span>Buscar</span></>}
                </button>
              </div>
            </div>
          </div>
              ...
              {selectedFatura && (
                 <div className="glass-panel animate-slide-in-right" style={{ padding: '1.5rem', background: '#f8fafc', border: '1px solid #e2e8f0', position: 'sticky', top: '20px', height: 'fit-content' }}>
                    ...
                    <div style={{ marginTop: '2rem', display: 'flex', gap: '1rem' }}>
                       <button className="btn-secondary" style={{ flex: 1, justifyContent: 'center', background: '#10b981', color: 'white', borderColor: '#10b981' }} onClick={() => handleExportarERP(selectedFatura.id)}>
                          <FileText size={18} /> Atualiza NFSe
                       </button>
      ...
      {isFaturaModalOpen && (
        <div className="os-modal-overlay" onClick={() => setIsFaturaModalOpen(false)}>
          <div className="premium-modal-content full-screen" onClick={e => e.stopPropagation()}>
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                {(faturaModalMode === 'edit' || faturaModalMode === 'view') && (
                  <>
                    <button 
                      className="btn-secondary" 
                      onClick={() => handleExportarERP(editingFatura?.id)} 
                      disabled={!!(editingFatura?.id_fatura_erp && editingFatura?.id_fatura_erp !== 'null' && editingFatura?.id_fatura_erp !== '0')}
                      style={{ 
                        background: (!editingFatura?.id_fatura_erp || editingFatura?.id_fatura_erp === 'null' || editingFatura?.id_fatura_erp === '0') ? '#3b82f6' : '#94a3b8', 
                        color: 'white', 
                        borderColor: (!editingFatura?.id_fatura_erp || editingFatura?.id_fatura_erp === 'null' || editingFatura?.id_fatura_erp === '0') ? '#3b82f6' : '#94a3b8', 
                        display: 'flex', 
                        alignItems: 'center', 
                        gap: '0.5rem', 
                        padding: '0.5rem 1rem',
                        cursor: (!editingFatura?.id_fatura_erp || editingFatura?.id_fatura_erp === 'null' || editingFatura?.id_fatura_erp === '0') ? 'pointer' : 'not-allowed'
                      }}>
                      <FileText size={18} /> Atualiza NFSe
                    </button>
                    <button className="btn-primary" onClick={() => handlePrintNFSe(editingFatura)} style={{ background: '#5C4033', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.5rem 1rem' }}>
                      <Printer size={18} /> Imprime NFSe
                    </button>
                ...
                <div className="animate-fade-in">
                  <div className="premium-master-panel" style={{ marginBottom: '1.5rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
                      ...
                      <div className="form-group">
                        <label><Hash size={14} /> ID GestãoClick</label>
                        <input 
                          type="text" 
                          className="form-input" 
                          value={faturaForm.id_fatura_erp || ''} 
                          readOnly 
                          style={{ background: '#f8fafc', fontWeight: 'bold', color: faturaForm.id_fatura_erp ? '#10b981' : '#94a3b8' }} 
                    ...
                    <div className="form-group span-2">
                      <div style={{ display: 'flex', alignItems: 'flex-end', gap: '0.75rem' }}>
                        ...
                        <button
                          ...
                          type="button"
                          onClick={handleGerarParcela}
                          disabled={faturaModalMode === 'view' || !faturaForm.id_planopag}
                          style={{ height: '42px', padding: '0 1rem', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', whiteSpace: 'nowrap' }}
                        >
                          GERAR PARCELA
                        </button>
                      </div>
            ...
            <div className="premium-modal-footer" style={{ display: 'flex', gap: '1rem', width: '100%', alignItems: 'center' }}>
              <div style={{ flex: 1, display: 'flex' }}>
                {(faturaModalMode === 'edit') && (
              ...
              {faturaModalMode !== 'view' && (
                activeFaturaModalTab === 'pneus' ? (
                  <button
                    className="btn-primary"
                    onClick={handleProceedToFaturaDetails}
                    disabled={loading}
                    style={{
                  ...
                  <button className="btn-primary" style={{ background: '#2563eb', display: 'flex', alignItems: 'center', gap: '0.5rem' }} onClick={handleSaveFatura}>
      ...
      {/* Single NFSe Print Layout */}
      {nfseToPrint && (
        <div className="nfse-print-wrapper only-print">
          <div className="nfse-print-header">
            <h2>Prefeitura Municipal de {empresa?.cidade || 'Sua Cidade'} - {empresa?.uf || 'UF'}</h2>
            <p>Secretaria Municipal da Fazenda<br/>Estado do {empresa?.uf || 'UF'}<br/>www.fazendariogrande.pr.gov.br</p>
          </div>



          <table className="nfse-print-table" style={{ border: '3px solid black' }}>
            <thead>
              <tr>
                <th style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>Número do RPS</th>
                <th style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>Série NF</th>
                <th style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>Número da Nota</th>
                <th style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>Data de Emissão</th>
                <th style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>Data Fato Gerador</th>
                <th style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>Código Verificacao</th>
                <th style={{ backgroundColor: '#f0f0f0', textAlign: 'center' }}>Folha</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td style={{ textAlign: 'center' }}>{nfseToPrint.id}</td>
                <td style={{ textAlign: 'center' }}>9</td>
                <td style={{ textAlign: 'center' }}>{nfseToPrint.numfatura || nfseToPrint.id}</td>
                <td style={{ textAlign: 'center' }}>{new Date().toLocaleDateString()}</td>
                <td style={{ textAlign: 'center' }}>{new Date(nfseToPrint.datafat).toLocaleDateString()}</td>
          ...
          <div style={{ display: 'flex', border: '3px solid black', marginBottom: '15px', color: 'black' }}>
            <div style={{ width: '180px', borderRight: '3px solid black', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 'bold', fontSize: '16px', padding: '5px' }}>
              <img src={logoEmpresa} alt="Logo Empresa" style={{ maxWidth: '100%', maxHeight: '80px', objectFit: 'contain' }} />
            </div>
            <div style={{ flex: 1, padding: '10px', display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <div><strong>Nome / Razão Social:</strong> {empresa?.razaosocial || 'TOTALCAP RECAPAGEM'}</div>
              <div style={{ display: 'flex', gap: '20px' }}>
                <div><strong>CNPJ:</strong> {empresa?.cnpj || '00.000.000/0000-00'}</div>
                <div><strong>Inscrição Municipal:</strong> {empresa?.im || 'Isento'}</div>
                <div><strong>Telefone:</strong> {empresa?.telefone || '---'}</div>
              </div>
              <div><strong>Endereço:</strong> {empresa?.rua || ''}, {empresa?.numcasa || ''} - {empresa?.bairro || ''} - {empresa?.cidade || ''} - {empresa?.uf || ''}</div>
            </div>
          </div>

          <h3 style={{ marginTop: 0, marginBottom: '5px', fontSize: '16px', color: 'black' }}>Tomador de Serviços</h3>
          <div style={{ border: '3px solid black', padding: '10px', marginBottom: '15px', color: 'black' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
              <div><strong>Nome / Razão Social:</strong> {nfseToPrint.contato_nome || nfseToPrint.cliente_nome}</div>
              <div style={{ display: 'flex', gap: '20px' }}>
                <div><strong>CNPJ:</strong> {nfseToPrint.cpfcnpj || '---'}</div>
                <div><strong>Inscrição Estadual:</strong> {nfseToPrint.rgie || '---'}</div>
                <div><strong>Telefone:</strong> {nfseToPrint.fone || '---'}</div>
              </div>
              <div><strong>Endereço:</strong> {nfseToPrint.endereco || ''} - {nfseToPrint.cidade || ''} - {nfseToPrint.uf || ''}</div>
              <div><strong>Email:</strong> {nfseToPrint.email || '---'}</div>
              <div><strong>CEP:</strong> {nfseToPrint.cep || '---'}</div>
            </div>
          </div>

          <h3 style={{ marginTop: 0, marginBottom: '5px', fontSize: '16px', color: 'black' }}>Condição de Pagamento</h3>
          <div style={{ border: '3px solid black', padding: '10px', marginBottom: '15px', color: 'black' }}>
            <div>
              {nfseToPrint.parcelas?.length > 0 
                ? nfseToPrint.parcelas.map((p: any) => `(${p.num_parcela}) ${new Date(p.vencto).toLocaleDateString()} R$ ${parseFloat(p.valor || 0).toFixed(2)}`).join('   ')
                : 'À vista'}
            </div>
          </div>

          <h3 style={{ marginTop: '20px', marginBottom: '5px', fontSize: '16px', color: 'black' }}>Serviços Prestado</h3>
          <table className="nfse-print-table">
            <thead>
              <tr>
                <th>Item</th>
                <th>Descrição</th>
            ...
            <tbody>
              {nfseToPrint.pneus?.map((p: any, idx: number) => (
                <tr key={idx}>
                  <td>{idx + 1}</td>
                  <td>{p.servico_descricao || p.descricao} (Pneu #{p.id_pneu})</td>
                  <td>{parseFloat(p.quant || 1).toFixed(0)}</td>
                  <td>{parseFloat(p.valor || 0).toFixed(2)}</td>
                  <td>{parseFloat(p.vrtotal || 0).toFixed(2)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <table className="nfse-print-table">
            ...
            <tbody>
              <tr>
                <td colSpan={10}>Todos os valores: 0,00</td>
              </tr>
            </tbody>
          </table>

          <p><strong>Código dos Serviços:</strong> 1404 - Recauchutagem ou regeneração de Pneus<br/>
             <strong>CNAE:</strong> 2212900 - Reforma de Pneumáticos Usados</p>

          <h3>Totais</h3>
          <table className="nfse-print-table">
            <thead>
              <tr>
                <th>Valor Total das Deduções</th>
                <th>Base de Cálculo</th>
                <th>Alíquota</th>
                <th>Valor do ISS</th>
                <th>Valor ISS Retido</th>
                <th>Valor Líquido da Nota</th>
              </tr>
            </thead>
            <tbody>
              <tr>
                <td>{parseFloat(nfseToPrint.vrbonus || 0).toFixed(2)}</td>
                <td>{(parseFloat(nfseToPrint.vrservico || 0) - parseFloat(nfseToPrint.vrbonus || 0)).toFixed(2)}</td>
                <td>2%</td>
                <td>{((parseFloat(nfseToPrint.vrservico || 0) - parseFloat(nfseToPrint.vrbonus || 0)) * 0.02).toFixed(2)}</td>
                <td>0,00</td>
                <td>{parseFloat(nfseToPrint.vrtotal || 0).toFixed(2)}</td>
              </tr>
            </tbody>
          </table>

          <h3 style={{ marginTop: '20px', marginBottom: '5px', fontSize: '16px', color: 'black' }}>Informações Adicionais</h3>
          <div style={{ border: '3px solid black', padding: '10px', marginBottom: '15px', color: 'black', minHeight: '60px' }}>
            <p style={{ margin: 0 }}>{nfseToPrint.obs || `O.S: ${nfseToPrint.numos || ''} | Vendedor: ${nfseToPrint.vendedor_nome || ''}`}</p>
          </div>
        </div>
      )}
    </div>
