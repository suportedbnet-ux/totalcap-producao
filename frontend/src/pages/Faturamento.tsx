...
export default function Faturamento() {
  ...
  const calculateParcelasPreview = () => {
    ...
    // Não regera parcelas se estiver editando uma fatura existente (já tem parcelas salvas)
    if (editingFatura) {
      console.log('DEBUG: editando fatura existente, não regera parcelas');
      return;
    }
  ...
  const handleGerarParcela = () => {
    ...
    if (editingFatura) {
      alert('Não é possível regenerar parcelas de uma fatura já existente. Edite as parcelas manualmente.');
      return;
    }
  ...
  const handleExportarERP = async () => {
    ...
    try {
      for (const faturaId of selectedFaturaIds) {
        const fatura = faturas.find(f => f.id === faturaId);
        if (!fatura) continue;

        const payloadParam = {
          "Cabecalho": {
            "cCodIntOS": String(fatura.id),
            "cCodParc": "999",
            "cEtapa": "20",
            "dDtPrevisao": fatura.datafat ? fatura.datafat.split('T')[0].split('-').reverse().join('/') : new Date().toLocaleDateString('pt-BR'),
            "nCodCli": fatura.id_contato || 0,
            "nQtdeParc": fatura.parcelas ? fatura.parcelas.length : 1
          },
          "Departamentos": [],
          "Email": {
            "cEnvBoleto": "N",
          ...
          "InformacoesAdicionais": {
            ...
            "cCodCateg": "1.01.02",
            "cDadosAdicNF": `Fatura ID ${fatura.id} exportada via sistema`,
            "nCodCC": 11850365
          },
          "ServicosPrestados": [
            {
              ...
              "cDescServ": "Serviço Prestado Fatura",
              "cRetemISS": "N",
              "cTribServ": "01",
              "impostos": {
                "cRetemIRRF": "S",
              ...
              "nQtde": 1,
              "nValUnit": fatura.vrtotal || 0
            }
          ]
        };

        await api.post('/omie/proxy', {
          endpoint_url: "https://app.omie.com.br/api/v1/servicos/os/",
          call: "IncluirOS",
          param: [payloadParam]
        });
      }
      
      alert("Faturas exportadas com sucesso via API!");
    } catch (err: any) {
      console.error(err);
      alert(`Erro ao exportar faturas: ${err.message}`);
    }
  };
