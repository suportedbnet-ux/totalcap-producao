...
import { Package, Search, Plus, Trash2, Edit2, Eye, X, DollarSign, Shield, Info, ClipboardList, Printer, Camera, Loader2, AlertCircle, User, FilePlus, Calendar, Save, Hash } from 'lucide-react';
...
export default function ColetaPneus() {
  ...
  const [filterNumOs, setFilterNumOs] = useState('');
  ...
  const [medidaSearchTerm, setMedidaSearchTerm] = useState('');
  const [showMedidaDropdown, setShowMedidaDropdown] = useState(false);
  const [filteredMedidas, setFilteredMedidas] = useState<any[]>([]);
  const medidaSearchRef = useRef<HTMLDivElement>(null);

  const [marcaSearchTerm, setMarcaSearchTerm] = useState('');
  const [showMarcaDropdown, setShowMarcaDropdown] = useState(false);
  const [filteredMarcas, setFilteredMarcas] = useState<any[]>([]);
  const marcaSearchRef = useRef<HTMLDivElement>(null);

  const [desenhoSearchTerm, setDesenhoSearchTerm] = useState('');
  const [showDesenhoDropdown, setShowDesenhoDropdown] = useState(false);
  const [filteredDesenhos, setFilteredDesenhos] = useState<any[]>([]);
  const desenhoSearchRef = useRef<HTMLDivElement>(null);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (medidaSearchRef.current && !medidaSearchRef.current.contains(event.target as Node)) {
        setShowMedidaDropdown(false);
      }
      if (marcaSearchRef.current && !marcaSearchRef.current.contains(event.target as Node)) {
        setShowMarcaDropdown(false);
      }
      if (desenhoSearchRef.current && !desenhoSearchRef.current.contains(event.target as Node)) {
        setShowDesenhoDropdown(false);
      }
    };
  ...
  useEffect(() => {
    if (isPneuModalOpen && tempPneu.id_medida && medidas.length > 0) {
      const m = medidas.find(x => String(x.id) === String(tempPneu.id_medida));
      if (m && medidaSearchTerm !== m.descricao) {
        setMedidaSearchTerm(m.descricao);
      } else if (!m && tempPneu.medidanova && medidaSearchTerm !== tempPneu.medidanova) {
        setMedidaSearchTerm(tempPneu.medidanova);
      }
    }
    if (isPneuModalOpen && tempPneu.id_marca && marcas.length > 0) {
      const m2 = marcas.find(x => String(x.id) === String(tempPneu.id_marca));
      if (m2 && marcaSearchTerm !== m2.descricao) {
        setMarcaSearchTerm(m2.descricao);
      } else if (!m2 && tempPneu.marcanova && marcaSearchTerm !== tempPneu.marcanova) {
        setMarcaSearchTerm(tempPneu.marcanova);
      }
    }
    if (isPneuModalOpen && tempPneu.id_desenho && desenhos.length > 0) {
      const d = desenhos.find(x => String(x.id) === String(tempPneu.id_desenho));
      if (d && desenhoSearchTerm !== d.descricao) {
        setDesenhoSearchTerm(d.descricao);
      } else if (!d && tempPneu.desenhonovo && desenhoSearchTerm !== tempPneu.desenhonovo) {
        setDesenhoSearchTerm(tempPneu.desenhonovo);
      }
    }
  }, [
    isPneuModalOpen,
    tempPneu.id_medida, medidas, tempPneu.medidanova, medidaSearchTerm,
    tempPneu.id_marca, marcas, tempPneu.marcanova, marcaSearchTerm,
    tempPneu.id_desenho, desenhos, tempPneu.desenhonovo, desenhoSearchTerm
  ]);
  ...
  useEffect(() => {
    ...
    if (filterId.trim() !== '') {
      const targetId = filterId.trim();
      filtered = filtered.filter(c => String(c.id).includes(targetId));
    }

    // Filtro por Número da OS
    if (filterNumOs.trim() !== '') {
      const targetNumOs = filterNumOs.trim();
      filtered = filtered.filter(c => (c.numos ? String(c.numos) : '').includes(targetNumOs));
    }

    // Filtro por Nome do Cliente
    if (filterCliente.trim() !== '') {
      const lowerCliente = filterCliente.toLowerCase();
      filtered = filtered.filter(c => 
        (c.contato?.nome || '').toLowerCase().includes(lowerCliente) ||
        (c.nome || '').toLowerCase().includes(lowerCliente)
      );
    }
  ...
  }, [filterId, filterNumOs, filterCliente, startDate, endDate, coletas]);
  ...
  const handleMedidaSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setMedidaSearchTerm(value);
    if (value.length >= 3) {
      const filtered = medidas.filter(m => m.descricao.toLowerCase().includes(value.toLowerCase())).slice(0, 10);
      setFilteredMedidas(filtered);
      setShowMedidaDropdown(true);
    } else {
      setShowMedidaDropdown(false);
    }
    if (tempPneu.id_medida) handleTempPneuChange('id_medida', 0);
  };

  const handleSelectMedida = (medida: any) => {
    setMedidaSearchTerm(medida.descricao);
    setShowMedidaDropdown(false);
    handleTempPneuChange('id_medida', medida.id);
  };

  const handleMarcaSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setMarcaSearchTerm(value);
    if (value.length >= 3) {
      const filtered = marcas.filter(m => m.descricao.toLowerCase().includes(value.toLowerCase())).slice(0, 10);
      setFilteredMarcas(filtered);
      setShowMarcaDropdown(true);
    } else {
      setShowMarcaDropdown(false);
    }
    if (tempPneu.id_marca) handleTempPneuChange('id_marca', 0);
  };

  const handleSelectMarca = (marca: any) => {
    setMarcaSearchTerm(marca.descricao);
    setShowMarcaDropdown(false);
    handleTempPneuChange('id_marca', marca.id);
  };

  const handleDesenhoSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const value = e.target.value;
    setDesenhoSearchTerm(value);
    if (value.length >= 3) {
      const filtered = desenhos.filter(d => d.descricao.toLowerCase().includes(value.toLowerCase())).slice(0, 10);
      setFilteredDesenhos(filtered);
      setShowDesenhoDropdown(true);
    } else {
      setShowDesenhoDropdown(false);
    }
    if (tempPneu.id_desenho) handleTempPneuChange('id_desenho', 0);
  };

  const handleSelectDesenho = (desenho: any) => {
    setDesenhoSearchTerm(desenho.descricao);
    setShowDesenhoDropdown(false);
    handleTempPneuChange('id_desenho', desenho.id);
  };
  ...
  const openPneuModal = (index: number | null) => {
    if (index !== null) {
      ...
      const p = formData.pneus[index];
      setTempPneu({ ...p });
      const m = medidas.find(x => String(x.id) === String(p.id_medida));
      setMedidaSearchTerm(m ? m.descricao : (p.medidanova || ''));
      const m2 = marcas.find(x => String(x.id) === String(p.id_marca));
      setMarcaSearchTerm(m2 ? m2.descricao : (p.marcanova || ''));
      const d = desenhos.find(x => String(x.id) === String(p.id_desenho));
      setDesenhoSearchTerm(d ? d.descricao : (p.desenhonovo || ''));
    } else {
      setEditingPneuIndex(null);
      setMedidaSearchTerm('');
      setMarcaSearchTerm('');
      setDesenhoSearchTerm('');
      setTempPneu({
        id_medida: 0,
        id_marca: 0,
        id_desenho: 0,
  ...
  const handleGenerateOS = (id: number) => {
    ...
    const errors: string[] = [];
    if (!coleta.pneus || coleta.pneus.length === 0) {
      errors.push("A coleta não possui pneus vinculados.");
    } else {
      coleta.pneus.forEach((p, idx) => {
        const itemNum = idx + 1;
        const hasMedida = (p.id_medida && p.id_medida > 0) || (p.medidanova && p.medidanova.trim() !== "");
        const hasMarca = (p.id_marca && p.id_marca > 0) || (p.marcanova && p.marcanova.trim() !== "");
        const hasDesenho = (p.id_desenho && p.id_desenho > 0) || (p.desenhonovo && p.desenhonovo.trim() !== "");
        const hasRecap = (p.id_recap && p.id_recap > 0);
        ...
        if (!hasRecap) errors.push(`Pneu #${itemNum}: Tipo de Recapagem não informado.`);
      });
    }

    if (errors.length > 0) {
      alert(`Não é possível gerar a OS. Pendências encontradas:\n\n${errors.join('\n')}\n\nPor favor, edite a coleta e preencha os dados faltantes.`);
      return;
    }
  ...
  const activeFilters = [
    ...
    filterNumOs && `OS: ${filterNumOs}`,
    filterCliente && `Cliente: ${filterCliente}`,
    startDate && `Início: ${new Date(startDate + 'T00:00:00').toLocaleDateString('pt-BR')}`,
    endDate && `Fim: ${new Date(endDate + 'T00:00:00').toLocaleDateString('pt-BR')}`
  ].filter(Boolean).join(' | ');
  ...
  return (
    <div className="coleta-container fade-in">
      ...
      <div className="search-section-os glass-panel os-search-section no-print" style={{ margin: '0 2rem 1rem 2rem' }}>
        <div className="os-search-grid">
          <div className="form-group os-field">
            <label><Hash size={13} /> ID Interno</label>
            <input 
              type="number" 
              placeholder="Ex: 45" 
              value={filterId}
          ...
          <div className="form-group os-field">
            <label><ClipboardList size={13} /> Número da OS</label>
            <input 
              type="text" 
              placeholder="Ex: 5020" 
              value={filterNumOs}
              onChange={(e) => setFilterNumOs(e.target.value)}
              className="form-input"
            />
          </div>

          <div className="form-group os-field">
            <label><Calendar size={13} /> Data Inicial</label>
            <input 
              type="date" 
              value={startDate}
          ...
          <div className="form-group os-field">
            <label><Calendar size={13} /> Data Final</label>
            <input 
              type="date" 
              value={endDate}
          ...
          <div className="form-group os-field os-field-cliente">
            <label><User size={13} /> Nome do Cliente</label>
            <input 
              type="text" 
          ...
          <div className="form-group os-field os-field-button">
            <label className="os-button-label">&nbsp;</label>
            <button
              type="button"
              className="btn-search-producao os-filter-btn"
              onClick={fetchData}
            >
              <Search size={16} /> <span>Filtrar</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="glass-panel table-container overflow-hidden" style={{ margin: '0 2rem 2rem 2rem' }}>
        <div className="table-toolbar no-print" style={{ padding: '0.75rem 1.5rem' }}>
          <div className="table-title-info" style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            <ClipboardList size={20} />
            <h3 style={{ margin: 0 }}>Listagem de Coleta de Pneus</h3>
            <span className="count-badge">{filteredColetas.length} registros</span>
          </div>
        </div>
                        ...
                        {formData.pneus?.length === 0 ? (
                          ...
                          formData.pneus.map((p: any, idx: number) => (
                            <tr key={idx}>
                              ...
                              <td>{tiposRecap.find(t => String(t.id) === String(p.id_recap))?.descricao || '---'}</td>
        ...
        <div className="coleta-modal-overlay" onClick={() => setIsPneuModalOpen(false)}>
          <div className="pneu-detail-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="pneu-modal-scroll">
              <div className="form-group" ref={medidaSearchRef} style={{ position: 'relative' }}>
                <label>Medida</label>
                <input
                  type="text"
                  className="form-input"
                  placeholder="Digite 3 caracteres..."
                  value={medidaSearchTerm}
                  onChange={handleMedidaSearchChange}
                />
                {showMedidaDropdown && filteredMedidas.length > 0 && (
                  <div className="suggestions-dropdown glass-panel" style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 10 }}>
                    {filteredMedidas.map(m => (
                      <div key={m.id} className="suggestion-item" onClick={() => handleSelectMedida(m)}>
              ...
              <div className="form-group" ref={marcaSearchRef} style={{ position: 'relative' }}>
                <label>Marca</label>
                <input
                  type="text"
                  className="form-input"
                ...
                {showMarcaDropdown && filteredMarcas.length > 0 && (
                  <div className="suggestions-dropdown glass-panel" style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 10 }}>
                    {filteredMarcas.map(m => (
                      <div key={m.id} className="suggestion-item" onClick={() => handleSelectMarca(m)}>
                        <span>{m.descricao}</span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="form-group" ref={desenhoSearchRef} style={{ position: 'relative' }}>
                <label>Desenho</label>
                <input
                  type="text"
                  className="form-input"
                  placeholder="Digite 3 caracteres..."
                  value={desenhoSearchTerm}
                  onChange={handleDesenhoSearchChange}
                />
                {showDesenhoDropdown && filteredDesenhos.length > 0 && (
                  <div className="suggestions-dropdown glass-panel" style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 10 }}>
                    {filteredDesenhos.map(d => (
                      <div key={d.id} className="suggestion-item" onClick={() => handleSelectDesenho(d)}>
              ...
              <div className="form-group">
                ...
                <select className="form-input" value={String(tempPneu.id_recap || '0')} onChange={e => handleTempPneuChange('id_recap', e.target.value)}>
                  <option value="0">Selecione...</option>
                  {tiposRecap.map(t => <option key={t.id} value={String(t.id)}>{t.descricao}</option>)}
                </select>
              </div>
              <div className="form-group">
                <label>Valor (R$)</label>
        ...
        <div className="report-list-header">
          ...
          <div className="report-title-bar">
            ...
            <div className="report-filter-info">
              {activeFilters ? `Filtro: "${activeFilters}"` : 'Filtro: Todos os registros'}
            </div>
          </div>
        </div>
