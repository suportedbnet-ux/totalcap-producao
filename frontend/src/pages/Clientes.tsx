...
export default function Clientes() {
  ...
  const handleExportarAPI = async () => {
    if (!formData.cpfcnpj) {
      alert("Para exportar para o ERP, é necessário preencher o CPF/CNPJ.");
      return;
    }
    ...
    try {
      const payloadParam = {
        "tipo_pessoa": formData.pessoa === 'F' ? 'PF' : 'PJ',
        "nome": formData.nome || "",
        "razao_social": formData.razaosocial || "",
        "cnpj": formData.pessoa === 'J' ? (formData.cpfcnpj || "") : "",
        "inscricao_estadual": formData.inscestadual || "",
        "inscricao_municipal": formData.inscmunicipal || "",
        "cpf": formData.pessoa === 'F' ? (formData.cpfcnpj || "") : "",
        "rg": formData.rg || "",
        "data_nascimento": formData.datanascto ? formData.datanascto.split('T')[0] : "",
        "telefone": formData.foneprincipal || "",
        "celular": formData.celular_comercial || "",
        "fax": "",
        ...
        "contatos": [
          {
            "contato": {
              "nome": formData.contato_comercial || "",
              "contato": formData.celular_comercial || "",
              "cargo": "",
              "observacao": "[-------]"
            }
          }
        ...
        "enderecos": [
          {
            "endereco": {
              ...
              "numero": formData.numcasa || "",
              "complemento": formData.complemento || "",
              "bairro": formData.bairro || "",
              "cidade_id": "",
              "nome_cidade": formData.cidade || "",
              "estado": formData.uf || ""
            }
          }
        ]
      ...
      const jsonPayload = JSON.stringify(payloadRequest, null, 2);
      ...
      setApiResponseInfo(`Requisição Enviada:\n${jsonPayload}\n\nRetorno da API:\n${jsonResponse}`);

      // Copia para a área de transferência
      try {
        await navigator.clipboard.writeText(jsonPayload);
        alert(`Ação concluída! Os detalhes da integração estão na tela.\n\n(O JSON da requisição foi copiado para a sua área de transferência para facilitar).`);
      } catch (err) {
        alert("Ação concluída! Os detalhes da integração estão na tela.");
      }
      ...
      const debugText = `ERRO NA REQUISIÇÃO\n\n=== REQUISIÇÃO ENVIADA ===\n${jsonPayload}\n\n=== RETORNO DE ERRO ===\n${jsonError}`;
      
      setApiResponseInfo(debugText);
      console.error("Erro Exportação ERP:", err);
      
      try {
        await navigator.clipboard.writeText(debugText);
        alert(`Erro ao exportar cliente. O JSON de envio e o detalhe do erro foram copiados para a sua área de transferência.`);
      } catch (e) {
        alert(getErrorMessage(err, "Erro ao exportar cliente para o ERP. Verifique os dados na tela."));
      }
    }
  };
  ...
  const handleImportarAPI = async () => {
    if (!formData.cpfcnpj) {
      alert("Por favor, preencha o campo CPF/CNPJ antes de importar.");
      return;
    }
    ...
    try {
      const payloadRequest = {
        recurso: "clientes",
        method: "GET",
        param: formData.pessoa === 'J' ? { cnpj: formData.cpfcnpj } : { cpf: formData.cpfcnpj }
      };

      const response = await api.post('/gestaoclick/proxy', payloadRequest);
      const rawData = response.data;
      const apiResp = rawData.response_data;

      let clientData: any = null;
      if (apiResp?.data && Array.isArray(apiResp.data)) {
        clientData = apiResp.data[0];
      } else if (Array.isArray(apiResp)) {
        clientData = apiResp[0];
      } else {
        clientData = apiResp;
      }

      if (clientData && (clientData.cnpj || clientData.cpf || clientData.nome)) {
        const endereco = clientData.enderecos?.[0]?.endereco || {};
        const contato = clientData.contatos?.[0]?.contato || {};

        setFormData((prev: any) => ({
          ...prev,
          id_contato_erp: clientData.id ? String(clientData.id) : prev.id_contato_erp,
          pessoa: clientData.tipo_pessoa === 'PJ' ? 'J' : 'F',
          nome: clientData.nome || prev.nome,
          razaosocial: clientData.razao_social || prev.razaosocial,
          cpfcnpj: clientData.cnpj || clientData.cpf || prev.cpfcnpj,
          inscestadual: clientData.inscricao_estadual || prev.inscestadual,
          inscmunicipio: clientData.inscricao_municipal || prev.inscmunicipio,
          rg: clientData.rg || prev.rg,
          datanascto: clientData.data_nascimento ? new Date(clientData.data_nascimento).toISOString() : prev.datanascto,
          foneprincipal: clientData.telefone || prev.foneprincipal,
          celular_comercial: clientData.celular || prev.celular_comercial,
          email: clientData.email || prev.email,
          ativo: clientData.ativo === '1' || clientData.ativo === 1 || clientData.ativo === true,
          contato_comercial: contato.nome || prev.contato_comercial,
          cep: endereco.cep || prev.cep,
          rua: endereco.logradouro || prev.rua,
          numcasa: endereco.numero || prev.numcasa,
          complemento: endereco.complemento || prev.complemento,
          bairro: endereco.bairro || prev.bairro,
          cidade: endereco.nome_cidade || prev.cidade,
          uf: endereco.estado || prev.uf
        }));
        
        alert("Dados importados com sucesso via ERP GestãoClick!");
      } else {
        alert("Cliente não encontrado ou formato de resposta inesperado.\nVerifique o retorno no console (F12).");
        console.warn("Resposta da API de Importação:", rawData);
      }
    } catch (err: any) {
      alert(getErrorMessage(err, "Erro ao consultar cliente no ERP GestãoClick."));
    }
  };
          ...
          <div className="premium-modal-content large" style={{ maxWidth: '1000px' }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="modal-form-scroll">
              <div className="modal-body">
                ...
                {apiResponseInfo && (
                  <div className="premium-master-panel" style={{ background: '#1e293b', color: '#f8fafc', padding: '1.5rem', marginBottom: '1.5rem', borderRadius: '12px', border: '1px solid #334155' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem', borderBottom: '1px solid #334155', paddingBottom: '0.8rem' }}>
                      <span style={{ fontWeight: 'bold', color: '#38bdf8', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        <Users size={18} /> Dados da Integração ERP
                      </span>
                ...
                {activeTab === 'geral' && (
                  <div className="tab-content" style={{ padding: '2rem' }}>
                    ...
                    <div className="grid-4">
                        ...
                        <div className="form-group">
                            <label>ID Contato ERP</label>
                            <input className="form-input" id="id_contato_erp" value={formData.id_contato_erp || ''} onChange={handleChange} placeholder="Código Omie" />
                        </div>
                    </div>
              ...
              <div className="premium-modal-footer no-print" style={{ width: '100%' }}>
                <div style={{ display: 'flex', gap: '10px', marginRight: 'auto' }}>
                  {modalMode !== 'view' && (
                    <button 
                      type="button" 
                      className="btn-primary" 
                      style={{ background: '#3b82f6', padding: '0.5rem 1rem', fontSize: '0.85rem', display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 600 }} 
                      onClick={handleImportarAPI}
                    >
                      <Search size={18} /> Importa ERP
                    </button>
                  )}
                </div>
