  ...
  const handleExportarAPI = async () => {
    ...
    try {
      const payloadCheck = {
        ...
        "codigo_cliente_integracao": formData.cpfcnpj
      };
      await api.post('/omie/proxy', {
        endpoint_url: "https://app.omie.com.br/api/v1/geral/clientes/",
        call: "ConsultarCliente",
        param: [payloadCheck]
      });
      
      // Se não deu erro, o cliente já existe!
      alert("Erro: Este cliente já existe no ERP da Omie. Não foi possível exportar um cliente duplicado.");
      return;
    } catch (e: any) {
      // Omie retorna erro 500 com faultstring quando não encontra o cliente.
      // Podemos prosseguir com a inclusão.
    }

    try {
      const payloadParam: Record<string, any> = {
        "codigo_cliente_omie": formData.id_contato_erp || "",
        "codigo_cliente_integracao": formData.cpfcnpj || "",
        "razao_social": formData.razaosocial || "",
        "cnpj_cpf": formData.cpfcnpj || "",
        "nome_fantasia": formData.nome || "",
        "telefone1_numero": formData.foneprincipal || "",
        "contato": formData.contato_comercial || "",
        "endereco": formData.rua || "",
        "endereco_numero": formData.numcasa || "",
        ...
        "telefone2_numero": formData.celular_comercial || "",
        "email": formData.email || "",
        "homepage": formData.site || "",
        "inscricao_estadual": formData.inscestadual || "",
        "inscricao_municipal": formData.inscmunicipal || "",
        "tipo_atividade": formData.id_atividade || "",
        "contribuinte": formData.contribuinte ? "S" : "N",
        "observacao": formData.obs || "",
        "obs_detalhadas": formData.ref_com || "",
        "pessoa_fisica": formData.pessoa === "F" ? "S" : "N",
        "cidade_ibge": formData.codigoibge || "",
        "valor_limite_credito": formData.limicredito || 0
      };

      Object.keys(payloadParam).forEach(k => {
        if (payloadParam[k] === "" || payloadParam[k] === null || payloadParam[k] === undefined) {
          delete payloadParam[k];
        }
      });

      const response: any = await api.post('/omie/proxy', {
        endpoint_url: "https://app.omie.com.br/api/v1/geral/clientes/",
        call: "IncluirCliente",
        param: [payloadParam]
      });
      ...
      if (data && data.codigo_cliente_omie) {
        setFormData((prev: any) => ({
          ...prev,
          id_contato_erp: data.codigo_cliente_omie
        }));
        alert(`Cliente exportado com sucesso! Código Omie: ${data.codigo_cliente_omie}`);
      } else {
        alert("Cliente exportado com sucesso!");
      }
    } catch (err: any) {
      alert(getErrorMessage(err, "Erro ao exportar cliente para o ERP."));
    }
  };
  ...
  const handleImportarAPI = async () => {
    ...
    try {
      ...
      if (data && !data.faultstring) {
        const isContrib = data.contribuinte === 'S';
        const isFisica = data.pessoa_fisica === 'S';
        
        setFormData((prev: any) => ({
          ...prev,
          id_contato_erp: data.codigo_cliente_omie || prev.id_contato_erp,
          cpfcnpj: data.cnpj_cpf || data.codigo_cliente_integracao || prev.cpfcnpj,
          razaosocial: data.razao_social || prev.razaosocial,
          nome: data.nome_fantasia || prev.nome,
          foneprincipal: data.telefone1_numero || prev.foneprincipal,
          contato_comercial: data.contato || prev.contato_comercial,
          rua: data.endereco || prev.rua,
          numcasa: data.endereco_numero || prev.numcasa,
          bairro: data.bairro || prev.bairro,
          complemento: data.complemento || prev.complemento,
          uf: data.estado || prev.uf,
          cidade: data.cidade || prev.cidade,
          cep: data.cep || prev.cep,
          codigopais: data.codigo_pais || prev.codigopais,
          celular_comercial: data.telefone2_numero || prev.celular_comercial,
          email: data.email || prev.email,
          site: data.homepage || prev.site,
          inscestadual: data.inscricao_estadual || prev.inscestadual,
          inscmunicipio: data.inscricao_municipal || prev.inscmunicipio,
          id_atividade: data.tipo_atividade ? parseInt(data.tipo_atividade) : prev.id_atividade,
          contribuinte: data.contribuinte ? isContrib : prev.contribuinte,
          obs: data.observacao || prev.obs,
          ref_com: data.obs_detalhadas || prev.ref_com,
          pessoa: data.pessoa_fisica ? (isFisica ? 'F' : 'J') : prev.pessoa,
          codigoibge: data.cidade_ibge ? parseInt(data.cidade_ibge) : prev.codigoibge,
          limicredito: data.valor_limite_credito ? parseFloat(data.valor_limite_credito) : prev.limicredito
        }));
        
        alert("Dados importados com sucesso via ERP!");
      } else {
        alert("Cliente não encontrado ou erro na importação.");
      }
    } catch (err: any) {
      alert(getErrorMessage(err, "Erro ao consultar cliente no ERP."));
    }
  };
