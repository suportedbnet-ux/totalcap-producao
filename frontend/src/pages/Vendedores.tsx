...
import { Plus, Search, Edit2, Trash2, X, ExternalLink, Printer, Target, Calendar, DollarSign, Check, ChevronLeft, Eye, Download } from 'lucide-react';
...
export default function Vendedores() {
  ...
  const [formData, setFormData] = useState({
    ...
    cargo: '',
    ativo: true,
    id_vendedor_erp: '' as string | number
  });
  ...
  const openModal = (mode: 'create' | 'edit', vendedor?: Vendedor) => {
    ...
    if (mode === 'edit' && vendedor) {
      ...
      setFormData({
        ...
        id_vendedor_erp: vendedor.id_vendedor_erp || ''
      });
    } else {
      setCurrentId(null);
      setFormData({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        id_vendedor_erp: formData.id_vendedor_erp ? String(formData.id_vendedor_erp) : null
      };
  ...
  const handleImportarERP = async () => {
    if (!formData.apelido) {
      alert("Para importar do ERP, é necessário ter um apelido preenchido.");
      return;
    }
    ...
    try {
      ...
      const payloadRequest = {
        recurso: `funcionarios?nome=${encodeURIComponent(formData.apelido)}`,
        method: "GET"
      };
      ...
      if (Array.isArray(responseData) && responseData.length > 0) {
        funcId = responseData[0].id;
      } else if (responseData && Array.isArray(responseData.data) && responseData.data.length > 0) {
        funcId = responseData.data[0].id;
      } else if (responseData && responseData.id) {
        funcId = responseData.id;
      }

      if (funcId) {
        setFormData(prev => ({ ...prev, id_vendedor_erp: funcId }));
        alert(`ERP Importado com sucesso. ID Vinculado: ${funcId}\n\nOs dados de requisição e resposta foram copiados para a área de transferência.`);
      } else {
        alert("Nenhum vendedor encontrado com este apelido no ERP.\n\nOs dados foram copiados para a área de transferência.");
      }
    } catch (error: any) {
      ...
      const debugData = {
        request: {
          recurso: `funcionarios?nome=${formData.apelido}`,
          method: "GET"
        },
            ...
            <div className="modal-body" style={{ background: '#E5E5E5', padding: '1.5rem', minHeight: '400px' }}>
              {activeTab === 'dados' ? (
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="premium-master-panel" style={{ background: '#FFFFFF', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)', maxHeight: 'calc(100vh - 300px)', overflowY: 'auto' }}>
                    <div className="form-grid">
                      ...
                      <div className="form-group">
                        <label htmlFor="id_vendedor_erp" style={{ fontWeight: '600', color: '#475569' }}>ID Vendedor ERP</label>
                        <input className="form-input" id="id_vendedor_erp" value={formData.id_vendedor_erp} onChange={handleChange} style={{ borderRadius: '8px', border: '1px solid #cbd5e1' }} disabled={modalMode === 'view'} />
                      </div>
                    </div>
                  </div>
