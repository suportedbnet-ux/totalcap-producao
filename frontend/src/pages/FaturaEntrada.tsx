          ...
          <div className="premium-modal-content full-screen" onClick={e => e.stopPropagation()}>
            <div className="premium-modal-header">
              ...
              <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
                {(modalMode === 'edit' || modalMode === 'view') && editingFatura && (
                  <>
                    <button className="btn-primary" style={{ background: '#6366f1', borderColor: '#6366f1', display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.4rem 1rem', fontSize: '0.85rem' }} onClick={() => handleExportarERP(editingFatura.id)} disabled={loading}>
                      <ExternalLink size={16} /> Exporta ERP
                    </button>
                    <button className="btn-secondary" onClick={() => window.print()} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.4rem 1rem', fontSize: '0.85rem' }}>
                      <Printer size={16} /> Imprime Entrada
                    </button>
                  </>
                )}
      ...
      <div className="print-block">
        <div className="print-header" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', borderBottom: '2px solid #334155', paddingBottom: '12px', marginBottom: '20px' }}>
          ...
          <h1 className="print-title" style={{ fontSize: '1.8rem', fontWeight: 700, color: '#1e293b', margin: 0 }}>
            Fatura de Entrada {editingFatura?.id ? `#${editingFatura.id}` : ''}
          </h1>
        </div>
        ...
        <div style={{ marginBottom: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', borderBottom: '1px solid #ccc', paddingBottom: '1rem' }}>
          <div>
            <strong>Cliente/Fornecedor:</strong> {faturaForm.cliente_nome}<br/>
            <strong>Data:</strong> {new Date(faturaForm.datafat).toLocaleDateString()}<br/>
            <strong>Observações:</strong> {faturaForm.obs || 'Nenhuma'}<br/>
          </div>
          <div style={{ textAlign: 'right' }}>
            <strong>Vendedor:</strong> {vendedores.find(v => v.id === faturaForm.id_vendedor)?.nome || '---'}<br/>
            <strong>Valor Total:</strong> R$ {parseFloat(faturaForm.vrtotal || 0).toFixed(2)}<br/>
          </div>
        </div>

        <div className="print-table-section">
          <h3 style={{ borderBottom: '1px solid #ccc', paddingBottom: '5px', marginBottom: '10px' }}>Itens (Produtos)</h3>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            ...
            <tbody>
              {produtosItems.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '1rem' }}>Nenhum item</td></tr>
              ) : (
                produtosItems.map((p, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: '8px 4px' }}>{p.codproduto || '-'}</td>
                    <td style={{ padding: '8px 4px' }}>{p.descricao}</td>
                    <td style={{ textAlign: 'center', padding: '8px 4px' }}>{p.quant}</td>
                    <td style={{ textAlign: 'right', padding: '8px 4px' }}>R$ {(parseFloat(p.valor) || 0).toFixed(2)}</td>
                    <td style={{ textAlign: 'right', padding: '8px 4px' }}>R$ {(parseFloat(p.vrtotal) || 0).toFixed(2)}</td>
                  </tr>
                ))
              )}
            </tbody>
