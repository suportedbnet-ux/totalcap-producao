...
export default function Produtos() {
  ...
  useEffect(() => {
    const performSearch = async () => {
      try {
        setLoading(true);
        const res = await api.get('/produtos/', { params: { search: debouncedSearch } });
    ...
    if (debouncedSearch.length >= 3) {
      performSearch();
    } else if (debouncedSearch === '' && produtos.length > 0) {
      fetchData();
    }
  }, [debouncedSearch]);
  ...
  const openModal = (mode: 'create' | 'edit', produto?: Produto) => {
    ...
    if (mode === 'edit' && produto) {
      ...
      setCurrentId(null);
      setFormData({ codprod: '', id_produto_erp: '', id_grupo: 0, descricao: '', unidade: '', precoven: 0, ativo: true });
    }
    setIsModalOpen(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { 
      ...formData, 
      id_produto_erp: formData.id_produto_erp ? Number(formData.id_produto_erp) : null,
      id_grupo: Number(formData.id_grupo) || null, 
      precoven: Number(formData.precoven) || 0 
    };
    try {
      if (modalMode === 'create') {
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body" style={{ background: '#E5E5E5', padding: '1.5rem' }}>
                ...
                <div className="premium-master-panel" style={{ background: '#FFFFFF', padding: '1.5rem', borderRadius: '12px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.1)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '1.2rem', alignItems: 'flex-start', marginTop: '1.2rem' }}>
                    <div className="form-group" style={{ flex: 1, marginTop: '2.2rem' }}>
                    ...
                    <div className="form-group" style={{ flex: 1 }}>
                      <label htmlFor="id_produto_erp" style={{ fontWeight: '600', color: '#475569' }}>ID Produto ERP</label>
                      <input type="number" className="form-input" id="id_produto_erp" value={formData.id_produto_erp} onChange={handleChange} placeholder="Ex: 12345" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid #cbd5e1', marginTop: '0.5rem' }} />
                    </div>
                  </div>
                </div>
              </div>
