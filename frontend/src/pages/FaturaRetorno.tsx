...
export default function FaturaRetorno() {
  ...
  const handleImportFaturaData = async (sourceFatura: any) => {
    ...
    try {
      ...
      const returnProducts = (fullFatura.pneus || []).map((p: any) => {
        const partes = [];
        if (p.medida_nome) partes.push(p.medida_nome.trim());
        if (p.marca_nome) partes.push(p.marca_nome.trim());
        if (p.numserie) partes.push(`Serie:${p.numserie.trim()}`);
        if (p.dot) partes.push(`Dot:${p.dot.trim()}`);
        
        return {
          codproduto: 'Pneu',
          descricao: partes.join(' ') || 'Pneu',
  ...
  const handleUpdateProdutoRow = (index: number, field: string, value: any) => {
    ...
    if (field === 'codproduto') {
      const prod = allProdutos.find(p => p.codprod === value || p.codatual === value || p.descricao === value);
  ...
  const handleExportarERP = async (faturaIdOverride?: number) => {
    const idsToExport = faturaIdOverride ? [faturaIdOverride] : selectedFaturas;
    ...
    try {
      ...
      for (const faturaId of idsToExport) {
        ...
        const payloadParam: any = {
          ...
          "produtos": (fatura.produtos || []).map((fatura_produto: any) => {
            const produto = allProdutos.find(p => p.codprod === fatura_produto.codproduto || p.codatual === fatura_produto.codproduto) || {};
            const finalId = produto.id_produto_erp ? String(produto.id_produto_erp) : "164639281";
            return {
              "produto": {
                "produto_id": finalId,
                "variacao_id": "0",
                "nome_produto": "Pneu",
                "detalhes": fatura_produto.descricao || "Produto",
                "movimenta_estoque": "2",
                "possui_variacao": "0",
                "sigla_unidade": "UN",
                "quantidade": String(fatura_produto.quant || "1"),
                "tipo_valor_id": "90858",
                "nome_tipo_valor": "Atacado",
                "valor_custo": String(produto.valor || produto.Valor || "0.00"),
                "valor_venda": String(fatura_produto.valor || "0.00"),
                "tipo_desconto": "R$",
                "desconto_valor": "0.00",
                "desconto_porcentagem": "0.00",
                "valor_total": String(fatura_produto.vrtotal || "0.00")
              }
            };
          })
        };

        if (payloadParam.produtos && payloadParam.produtos.length === 0) {
          delete payloadParam.produtos;
        }
        ...
        if (erpId) {
          if (editingFatura && editingFatura.id === faturaId) {
      ...
      let errorDetail = err.response?.data?.detail || err.message;
      if (typeof errorDetail === 'object') {
        const textToCopy = JSON.stringify(errorDetail, null, 2);
        try {
          navigator.clipboard.writeText(textToCopy);
        } catch(e) {}
        errorDetail = textToCopy;
      }
      alert("Erro ao exportar (Detalhes copiados p/ área de transferência!): \n" + errorDetail);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="faturamento-container">
          ...
          <div className="premium-modal-content full-screen" onClick={e => e.stopPropagation()}>
            <div className="premium-modal-header">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                {editingFatura && (
                  <>
                    <button className="btn-primary" style={{ background: '#6366f1', borderColor: '#6366f1', padding: '0.4rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }} onClick={() => handleExportarERP(editingFatura.id)} disabled={loading}>
                      <ExternalLink size={18} /> Exporta ERP
                    </button>
                    <button className="btn-secondary" onClick={() => window.print()} style={{ padding: '0.4rem 1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          ...
                          <tr key={idx}>
                            <td>
                              ...
                              <datalist id="produtos-list">
                                {allProdutos.map(p => (
                                  <option key={p.id} value={p.codprod || p.codatual}>{p.descricao}</option>
                                ))}
                              </datalist>
                            </td>
                            <td>
      ...
      <div className="print-block">
        <div className="print-header" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', borderBottom: '2px solid #334155', paddingBottom: '12px', marginBottom: '20px' }}>
          ...
          <h1 className="print-title" style={{ fontSize: '1.8rem', fontWeight: 700, color: '#1e293b', margin: 0 }}>
            Fatura de Retorno {editingFatura?.id ? `#${editingFatura.id}` : ''}
          </h1>
        </div>
        
        <div style={{ marginBottom: '2rem', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', borderBottom: '1px solid #ccc', paddingBottom: '1rem' }}>
          <div>
            <strong>Cliente:</strong> {faturaForm.cliente_nome}<br/>
            <strong>Data:</strong> {new Date(faturaForm.datafat).toLocaleDateString()}<br/>
            <strong>Observações:</strong> {faturaForm.obs || 'Nenhuma'}<br/>
          </div>
          <div style={{ textAlign: 'right' }}>
            <strong>NF Entrada:</strong> {faturaForm.notaent || '---'}<br/>
            <strong>Chave Entrada:</strong> {faturaForm.chavent || '---'}<br/>
            <strong>Valor Total:</strong> R$ {parseFloat(faturaForm.vrtotal || 0).toFixed(2)}<br/>
          </div>
        </div>

        <div className="print-table-section">
          <h3 style={{ borderBottom: '1px solid #ccc', paddingBottom: '5px', marginBottom: '10px' }}>Produtos</h3>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.9rem' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #334155' }}>
                <th style={{ textAlign: 'left', padding: '8px 4px' }}>Cód</th>
                <th style={{ textAlign: 'left', padding: '8px 4px' }}>Descrição</th>
                <th style={{ textAlign: 'center', padding: '8px 4px' }}>Qte</th>
                <th style={{ textAlign: 'right', padding: '8px 4px' }}>V. Unit</th>
                <th style={{ textAlign: 'right', padding: '8px 4px' }}>Total</th>
              </tr>
            </thead>
            <tbody>
              {produtosItems.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '1rem' }}>Nenhum produto</td></tr>
              ) : (
                produtosItems.map((item, idx) => (
                  <tr key={idx} style={{ borderBottom: '1px solid #e2e8f0' }}>
                    <td style={{ padding: '8px 4px' }}>{item.codproduto}</td>
                    <td style={{ padding: '8px 4px' }}>{item.descricao}</td>
                    <td style={{ textAlign: 'center', padding: '8px 4px' }}>{item.quant}</td>
                    <td style={{ textAlign: 'right', padding: '8px 4px' }}>R$ {parseFloat(item.valor || 0).toFixed(2)}</td>
                    <td style={{ textAlign: 'right', padding: '8px 4px', fontWeight: 'bold' }}>R$ {parseFloat(item.vrtotal || 0).toFixed(2)}</td>
                  </tr>
                ))
              )}
            </tbody>
