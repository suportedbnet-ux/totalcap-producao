  ...
  useEffect(() => {
    const handleSearch = async () => {
      if (searchTerm.trim() === '') {
        ...
        // Busca direto no backend para garantir que ache registros que não foram lidos na primeira carga
        try {
          setLoading(true);
          const response = await api.get(`/cidades/?search=${encodeURIComponent(searchTerm)}`);
    ...
    // Aplica um delay para não fazer requisições a cada letra digitada
