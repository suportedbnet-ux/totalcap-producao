...
import { 
  ...
  Briefcase, Loader2, AlertCircle, FileText, Printer
} from 'lucide-react';
...
export default function Orcamento() {
  ...
  const fetchData = async () => {
    try {
      const [resClie, resVend, resMed, resDes, resServ] = await Promise.all([
        api.get('/clientes/'),
        api.get('/vendedores/'),
        api.get('/medidas/'),
        api.get('/desenhos/'),
  ...
  const fetchOrcamentos = async () => {
    try {
      ...
      if (response.data.length === 0 && (searchId || searchCliente || searchStartDate || searchEndDate)) {
        setSearchError('Nenhum resultado encontrado para os filtros aplicados.');
      }
    } catch (err) {
      console.error("Erro ao buscar orçamentos:", err);
      setSearchError('Erro ao buscar orçamentos.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    setIsSearching(true);
    fetchOrcamentos().finally(() => setIsSearching(false));
  };
  ...
  const handleClienteChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length > 1) {
      ...
      setFilteredClientes(filtered.slice(0, 8));
  ...
  return (
    <div className="p-6">
      ...
      <div className="search-section-os glass-panel orc-search-section">
        <form onSubmit={handleSearch} className="search-form-producao">
          <div className="orc-search-grid">
            <div className="form-group orc-field">
              <label><Hash size={13} /> ID</label>
              <input
                type="number"
            ...
            <div className="form-group orc-field orc-field-cliente">
              ...
              {showClientSuggestions && filteredClientes.length > 0 && (
                <div className="suggestions-dropdown glass-panel orc-suggestions">
            ...
            <div className="form-group orc-field orc-field-button">
              <label className="orc-button-label">&nbsp;</label>
              <button type="submit" className="btn-search-producao orc-filter-btn" disabled={isSearching} translate="no">
                {isSearching ? <span>Buscando...</span> : <><Search size={16} /> <span>Filtrar</span></>}
              </button>
            </div>
          </div>
        </form>
        {searchError && <div className="search-error" style={{ marginTop: '0.65rem', color: '#ef4444', fontSize: '0.85rem' }}>{searchError}</div>}
      </div>
          ...
          <div className="premium-modal-content full-screen" onClick={e => e.stopPropagation()}>
            <div className="premium-modal-header">
              ...
              <div style={{ display: 'flex', gap: '0.8rem', alignItems: 'center' }}>
                {editingOrcamento && (
                  <button className="btn-secondary" onClick={() => handlePrintProposta(formData)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.4rem 1rem', fontSize: '0.85rem' }}>
                    <Printer size={16} /> Imprime Orçamento
                  </button>
                )}
