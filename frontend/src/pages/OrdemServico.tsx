...
export default function OrdemServico() {
  ...
  useEffect(() => {
    if (isPneuModalOpen && tempPneu.id_medida && medidas.length > 0) {
      ...
      } else if (!m && tempPneu.medida_nome && medidaSearchTerm !== tempPneu.medida_nome) {
        setMedidaSearchTerm(tempPneu.medida_nome);
      }
    }
    if (isPneuModalOpen && tempPneu.id_marca && marcas.length > 0) {
      ...
      } else if (!m2 && tempPneu.marca_nome && marcaSearchTerm !== tempPneu.marca_nome) {
        setMarcaSearchTerm(tempPneu.marca_nome);
      }
    }
    if (isPneuModalOpen && tempPneu.id_desenho && desenhos.length > 0) {
      ...
      } else if (!d && tempPneu.desenho_nome && desenhoSearchTerm !== tempPneu.desenho_nome) {
        setDesenhoSearchTerm(tempPneu.desenho_nome);
      }
    }
  }, [
    isPneuModalOpen,
    tempPneu.id_medida, medidas, tempPneu.medida_nome, medidaSearchTerm,
    tempPneu.id_marca, marcas, tempPneu.marca_nome, marcaSearchTerm,
    tempPneu.id_desenho, desenhos, tempPneu.desenho_nome, desenhoSearchTerm
  ]);
  ...
  const handleMedidaSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (tempPneu.id_medida) {
      handleTempPneuChange('id_medida', 0);
    }
  };
  ...
  const openPneuModal = (index: number | null) => {
    // If any lookup is empty, fetchLookups and then set state after fetchLookups resolves
    if (medidas.length === 0 || marcas.length === 0 || desenhos.length === 0 || servicos.length === 0 || tiposRecap.length === 0) {
      fetchLookups().then(() => {
        // Now set the state for the modal
        if (index !== null) {
          setEditingPneuIndex(index);
          const p = formData.pneus[index];
          setTempPneu({ ...p });
          const m = medidas.find(x => String(x.id) === String(p.id_medida));
          setMedidaSearchTerm(m ? m.descricao : (p.medida_nome || ''));
          const m2 = marcas.find(x => String(x.id) === String(p.id_marca));
          setMarcaSearchTerm(m2 ? m2.descricao : (p.marca_nome || ''));
          const d = desenhos.find(x => String(x.id) === String(p.id_desenho));
          setDesenhoSearchTerm(d ? d.descricao : (p.desenho_nome || ''));
        } else {
      ...
      // Lookups are already loaded, proceed as before
      if (index !== null) {
        setEditingPneuIndex(index);
        const p = formData.pneus[index];
        setTempPneu({ ...p });
  ...
  const handleTempPneuChange = (field: string, value: any) => {
    setTempPneu(prev => {
      ...
      // Limpa o nome antigo para forçar o grid a buscar o nome novo via ID
      if (field === 'id_medida') next.medida_nome = '';
      if (field === 'id_marca') next.marca_nome = '';
      if (field === 'id_desenho') next.desenho_nome = '';
      if (field === 'id_servico') next.servico_nome = '';
      if (field === 'id_recap') next.tiporecap_nome = '';
      return next;
    });
  };
  ...
  return (
    <div className="os-container">
      ...
      <div className="search-section-os glass-panel os-search-section">
        <form onSubmit={handleSearch} className="search-form-producao">
          <div className="os-search-grid">
            ...
            <div className="form-group os-field">
              <label><FileText size={13} /> Número da OS</label>
              <input
                type="text"
                className="form-input"
                placeholder="Ex: 5020"
            ...
            <div className="form-group os-field os-field-button">
              ...
              <button type="submit" className="btn-search-producao os-filter-btn" disabled={isSearching} translate="no">
                {isSearching ? <span>Buscando...</span> : <><Search size={16} /> <span>Filtrar</span></>}
              </button>
            </div>

            {showSuggestions && filteredClientes.length > 0 && (
              <div className="suggestions-dropdown glass-panel os-suggestions">
      ...
      <div className="glass-panel table-container">
        <div className="table-toolbar" style={{ padding: '0.75rem 1.5rem' }}>
          <div className="table-title-info">
            <Package size={20} />
            <h3 style={{ margin: 0 }}>Listagem de Ordens de Serviço</h3>
            ...
            <form onSubmit={handleSubmit}>
              <div className="modal-body scrollable">
                <div className="premium-master-panel">
                  <div className="premium-section-title" style={{ display: 'flex', alignItems: 'center', gap: '0.9rem' }}>
                    ...
                    <span>Dados Principais e Cliente</span>
                  </div>
          ...
          <div className="modal-content pneu-detail-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              <div className="pneu-form-grid">
                <div className="form-group span-2" ref={medidaSearchRef} style={{ position: 'relative' }}>
                  <label>Medida *</label>
                  <input
                    type="text"
                    className="form-input select-blue"
                ...
                <div className="form-group span-2" ref={marcaSearchRef} style={{ position: 'relative' }}>
                  <label>Marca *</label>
                  <input
                    type="text"
                    className="form-input"
                ...
                <div className="form-group span-2" ref={desenhoSearchRef} style={{ position: 'relative' }}>
                  <label>Desenho</label>
                  <input
                    type="text"
                    className="form-input select-blue"
                ...
                <div className="form-group span-2">
                  ...
                  <select className="form-input select-blue" value={String(tempPneu.id_recap || '0')} onChange={(e) => handleTempPneuChange('id_recap', e.target.value)} disabled={modalMode === 'view'}>
                    <option value="0">Selecione o tipo...</option>
                    {tiposRecap.map(tr => <option key={tr.id} value={String(tr.id)}>{tr.descricao}</option>)}
                  </select>
                </div>

                <div className="form-group span-4">
                  <label>Serviço Principal</label>
                  <select className="form-input" value={tempPneu.id_servico ? String(tempPneu.id_servico) : "0"} onChange={(e) => handleTempPneuChange('id_servico', e.target.value)} disabled={modalMode === 'view'}>
                    <option value="0">Selecione o serviço...</option>
                    {servicos.map(s => <option key={s.id} value={String(s.id)}>{s.descricao}</option>)}
                  </select>
                </div>
