...
from backend.app.models.fatura import Fatura as FaturaModel, FaturaServico as FaturaServicoModel, FaturaParcela as FaturaParcelaModel, FaturaProduto as FaturaProdutoModel
...
def map_fatura_to_response(fatura):
    if hasattr(fatura, 'contato') and fatura.contato:
        ...
        erp_id = getattr(fatura.contato, 'id_contato_erp', None)
        fatura.id_contato_erp = str(erp_id) if erp_id is not None else None
    if hasattr(fatura, 'vendedor') and fatura.vendedor:
        fatura.vendedor_nome = fatura.vendedor.nome
        erp_vendedor_id = getattr(fatura.vendedor, 'id_vendedor_erp', None)
        fatura.id_vendedor_erp = str(erp_vendedor_id) if erp_vendedor_id is not None else None

    if hasattr(fatura, 'id_fatura_erp') and fatura.id_fatura_erp is not None:
        fatura.id_fatura_erp = str(fatura.id_fatura_erp)
    
    if hasattr(fatura, 'items') and fatura.items:
        for item in fatura.items:
            if hasattr(item, 'servico') and item.servico:
                erp_servico_id = getattr(item.servico, 'id_servico_erp', None)
                item.id_servico_erp = str(erp_servico_id) if erp_servico_id is not None else None
            else:
                item.id_servico_erp = None

    if hasattr(fatura, 'pneus') and fatura.pneus:
        for pneu in fatura.pneus:
...
def read_faturas(
    ...
    limit: int = 100,
    q: Optional[str] = None,
    tipofat: Optional[str] = None
) -> Any:
    query = db.query(FaturaModel).options(
        joinedload(FaturaModel.pneus).joinedload(PneuModel.servico),
        joinedload(FaturaModel.pneus).joinedload(PneuModel.medida),
        joinedload(FaturaModel.pneus).joinedload(PneuModel.marca),
        joinedload(FaturaModel.contato),
        joinedload(FaturaModel.vendedor),
        joinedload(FaturaModel.items).joinedload(FaturaServicoModel.servico),
        joinedload(FaturaModel.produtos),
    ...
    if tipofat:
        query = query.filter(FaturaModel.tipofat == tipofat)
...
def create_fatura(
    *,
    ...
    if fatura_in.pneu_ids:
        for pneu_id in fatura_in.pneu_ids:
            ...
            if pneu:
                ...
                pneu.id_fatura = db_fatura.id  # Vincula o pneu à fatura
    ...
    if fatura_in.produtos:
        for p in fatura_in.produtos:
            item_fatura = FaturaProdutoModel(
                id_fatura=db_fatura.id,
                codproduto=p.get('codproduto'),
                descricao=p.get('descricao'),
                valor=p.get('valor', 0),
                quant=p.get('quant', 1),
                vrtotal=p.get('vrtotal', 0),
                id_usuario=None
            )
            db.add(item_fatura)
...
def update_fatura(
    *,
    ...
    fatura_data = fatura_in.model_dump(exclude_unset=True, exclude={'pneu_ids', 'parcelas', 'produtos'})
    ...
    if fatura_in.pneu_ids is not None:
        # Ao editar e selecionar apenas alguns pneus, precisamos garantir que
        # os registros em `fatura_servico` sejam recriados SOMENTE para a nova
        # seleção.
        db.query(FaturaServicoModel).filter(FaturaServicoModel.id_fatura == db_fatura.id).delete()
        # Desvincular pneus atuais
        for pneu in db_fatura.pneus:
            pneu.statusfat = False
            pneu.id_fatura = None  # Remove o vínculo com a fatura
    ...
    if fatura_in.produtos is not None:
        db.query(FaturaProdutoModel).filter(FaturaProdutoModel.id_fatura == db_fatura.id).delete()
        db.flush()
