...
from backend.app.models.coleta import MobOS
...
def read_ordens_servico(
    ...
    # Filtro por intervalo de datas (dataentrada)
    # Espera datas no formato YYYY-MM-DD (valor do input[type=date])
    if start_date:
        query = query.filter(OSModel.dataentrada >= start_date + " 00:00:00")
    if end_date:
...
def create_ordem_servico(
    *,
    ...
    db_os.id_coleta = os_in.id_coleta
    ...
    # Reload with pneus and map relationships
    return read_ordem_servico(db_os.id, db)
...
def update_ordem_servico(
    *,
    ...
    return read_ordem_servico(os_id, db)
