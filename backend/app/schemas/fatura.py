...
class FaturaBase(BaseModel):
    ...
    tipofat: Optional[str] = '1'  # Tipo de fatura: 1=serviço
    id_fatura_erp: Optional[str] = None
    id_fatura_nfe: Optional[int] = None
    id_contato: int
...
class FaturaCreate(FaturaBase):
    ...
    produtos: Optional[List[dict]] = None # Produtos adicionados na fatura (Retorno/etc)
...
class FaturaServicoResponse(FaturaServicoBase):
    id: int
    id_fatura: int
    id_servico_erp: Optional[str] = None
...
class FaturaProdutoResponse(BaseModel):
    id: int
    id_fatura: int
    codproduto: Optional[str] = None
    descricao: Optional[str] = None
    quant: Optional[Decimal] = Decimal("1.00")
    valor: Optional[Decimal] = Decimal("0.00")
    vrtotal: Optional[Decimal] = Decimal("0.00")
